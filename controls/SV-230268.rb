...
control 'SV-230268' do
  title 'RHEL 8 must enable kernel parameters to enforce discretionary '\
    'access control on hardlinks.'
  desc 'Discretionary Access Control (DAC) is based on the notion that '\
    'individual users are "owners" of objects and therefore have '\
    'discretion over who should be authorized to access the object and in '\
    'which mode (e.g., read or write). Ownership is usually acquired as a '\
    'consequence of creating the object or via specified ownership '\
    'assignment. DAC allows the owner to determine who will have access to '\
    'objects they control. An example of DAC includes user-controlled file '\
    'permissions.
    ...
    'the information to any subject at the same sensitivity level. The policy is '\
    'bounded by the information system boundary. Once the information is passed '\
    'outside the control of the information system, additional means may be '\
    'required to ensure the constraints remain in effect. While the older, more '\
    ...
    'create soft or hard links to files they do not own. Disallowing such hardlinks '\
    'mitigate vulnerabilities based on insecure file system accessed by privileged '\
    'programs, avoiding an exploitation vector exploiting unsafe use of open() or '\
    'creat().

    The sysctl --system command will load settings from all system configuration '\
    'files. All configuration files are sorted by their filename in lexicographic '\
    'order, regardless of which of the directories they reside in. If multiple '\
    'files specify the same option, the entry in the file with the lexicographically '\
    'latest name will take precedence. Files are read from directories in the '\
    'following list from top to bottom. Once a file of a given filename is loaded, '\
  ...
  desc 'check', 'Verify the operating system is configured to enable DAC on '\
  ...
  $ sudo grep -r fs.protected_hardlinks /run/sysctl.d/*.conf '\
  ...
  If "fs.protected_hardlinks" is not set to "1", is missing or commented out, '\
  'this is a finding.
  ...
  Remove any configurations that conflict with the above from the following '\
  'locations:
  /run/sysctl.d/*.conf
  /usr/local/lib/sysctl.d/*.conf
  /usr/lib/sysctl.d/*.conf
