...
control 'SV-251708' do
  ...
  desc 'If RHEL 8 were to allow any user to make changes to software libraries, ' \
       'then those changes might be implemented without undergoing the ' \
       'appropriate testing and approvals that are part of a robust change ' \
       'management process. This requirement applies to RHEL 8 with software ' \
       'libraries that are accessible and configurable, as in the case of ' \
       'interpreted languages. Software libraries also include privileged ' \
       'programs that execute with escalated privileges. Only qualified and ' \
       'authorized individuals will be allowed to obtain access to information ' \
  ...
  desc 'fix', 'Configure the system-wide shared library directories within ' \
              '(/lib, /lib64, /usr/lib and /usr/lib64) to be protected from ' \
              'unauthorized access. Run the following command, replacing ' \
              '"[DIRECTORY]" with any library directory not owned by "root".
