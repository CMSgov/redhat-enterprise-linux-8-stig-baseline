...
are not routable.  There is not an equivalent filter for IPv6 traffic.'
  desc 'check', 'Verify RHEL 8 uses reverse path filtering on all IPv4 interfaces with the
following commands:
...
returned, this is a finding.'
  desc 'fix', 'Configure RHEL 8 to use reverse path filtering on all IPv4 interfaces by
