control 'SV-230459' do
  title "Successful/unsuccessful uses of the fchown system call in RHEL 8 must
...
information system (e.g., module or policy filter). The \"fchown\" system call
is used to change the ownership of a file referred to by the open file
descriptor.
...
attempts to use the \"fchown\" system call by performing the following command
to check the file system rules in \"/etc/audit/audit.rules\":

    $ sudo grep -w fchown /etc/audit/audit.rules

    -a always,exit -F arch=b32 -S fchown -F auid>=1000 -F auid!=unset -k
perm_mod
    -a always,exit -F arch=b64 -S fchown -F auid>=1000 -F auid!=unset -k
perm_mod
...
successful/unsuccessful use of the \"fchown\" system call by adding or updating
...
perm_mod
  ...
  tag fix_id: 'F-33103r568124_fix'
  tag cci: ['CCI-000169']
  tag nist: ['AU-12 a']

  audit_syscall = 'fchown'
